...
def run_buffetSaveCsv():
    
    # 1. 데이터 로드
    gdp_df = pd.read_csv("ml_pipeline/data/macro_data_gdp_full.csv", parse_dates=["date"])
    pred_df = pd.read_csv("ml_pipeline/data/recent_gdp_prediction.csv", parse_dates=["date"])
    wilshire_df = pd.read_csv("ml_pipeline/data/wilshire5000_yahoo_api.csv")
